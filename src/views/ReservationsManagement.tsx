...
const ReservationsManagement = () => {
  ...
  const { getReservations, addNewReservation, deleteReservation } =
    useReservations();
  ...
  useEffect(() => {
    ...
    const assignedFactories = selectedPersons
      ...
      .map((factoryName) =>
        factoriesData?.find((factory) => factory.name === factoryName)
      )
      .filter((factory) => factory !== undefined) as Factory[];

    const uniqueAssignedFactories = Array.from(
      new Set(assignedFactories.map((f) => f.name))
    ).map((name) =>
      assignedFactories.find((f) => f.name === name)
    ) as Factory[];
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
              ...
              >
                {personnelData &&
                  personnelData.map((personnel) => (
                    <MenuItem value={personnel.personalId}>
                      {personnel.name}
                    </MenuItem>
                  ))}
              </Select>
          ...
          >
            <DateTimePicker
              ...
              value={startTime}
              onChange={(newValue) =>
                handleTimeChange('start', newValue as Date)
              }
          ...
          <Box>
            <Button
              ...
              disabled={
                reservationsMutation.isPending ||
