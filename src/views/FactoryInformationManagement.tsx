...
const FactoryInformationManagement = () => {
  ...
  const handleAddNewFactoryClick = () => {
    ...
    if(factoriesExist && factories?.some(factoryName => factoryName.name.toUpperCase() === newFactoryName.toUpperCase())) {
  ...
  return (
    <Box>
      ...
      <Box>
        <TextField label="Factory Name" variant="outlined" sx={{ mt: 2, mr: 2}} value={newFactoryName} onChange={(e) => setNewFactoryName(e.target.value)}/>
        <TextField label="Timezone" variant="outlined" sx={{ mt: 2, mr: 2}} value={newFactoryTimezone} onChange={(e) => setNewFactoryTimezone(e.target.value)}/>
      <Button variant="contained" sx={{ mt: 2, mb: 4 }} onClick={handleAddNewFactoryClick} disabled={addNewFactoryMutation.isPending || !newFactoryName || !newFactoryTimezone}>
        Add Factory
      </Button>
      </Box>
      <Box>
        ...
        {factories?.map((factory) => (
          <Box key={factory.name} sx={{ display: "flex", flexDirection: "row", mb: 2, p: 2, border: "1px solid black", borderRadius: "8px"}}>
            <Box sx={{ my: "auto"}}>
              <Typography variant="h6">Name: {factory.name}</Typography>
              <Typography>Timezone: {factory.timezone}</Typography>
            </Box>
              <Button variant="outlined" sx={{ mt: 1, width: "150px", ml: "auto"}} color="error" onClick={() => handleDeleteFactoryClick(factory)}> Delete Factory</Button>
          </Box>
        ))}
      </Box>
    </Box>
